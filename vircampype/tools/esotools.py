...
def build_phase3_stacks(stacks_images, stacks_catalogs, mag_saturation):
    ...
    for idx_file in range(len(stacks_images)):
        ...
        for idx_hdu_stk, idx_hdu_ctg in zip(
            ...
            final_cols = _make_phase3_columns(
                data=tabledata,
                photometric_error_floor=setup.photometric_error_floor,
            )
...
def _make_phase3_columns(
    data: Union[np.recarray, Table], photometric_error_floor: float = 0.0
):
    """
    ...
    Parameters
    ----------
    data : Union[np.recarray, Table]
        Numpy array that allows field access using attributes.
    photometric_error_floor : float, optional
        Internal photometric error (mag) added in quadrature to the aperture/auto
        magnitude errors, matching :func:`convert2public`. Default 0.0.
    ...
    magerr_auto = np.sqrt(data["MAGERR_AUTO"] ** 2 + photometric_error_floor**2)
