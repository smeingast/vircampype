...
# Search for files
paths_files_j = sorted(glob(path_data + "Lupus_deep_J*/*.fits"))
paths_files_h = sorted(glob(path_data + "Lupus_deep_H*/*.fits"))
paths_files_k = sorted(glob(path_data + "Lupus_deep_K*/*.fits"))

# Write scripts for J
write_scripts(paths_files=paths_files_j, path_pype=path_pype, path_scripts=path_scripts, archive=False,
              projection="Lupus_deep", additional_source_masks=None, n_jobs=18,
