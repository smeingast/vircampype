...
class AstrometricCalibratedSextractorCatalogs(SextractorCatalogs):
    ...
    def crunch_source_catalogs(self):
        ...
        for idx_file in range(self.n_files):
            ...
            [
                t.add_column(
                    (t["MAG_APER"].data[:, -1] - t["MAG_APER"].data.T).T,
                    name="MAG_APER_COR",
                )
            ...
            for par in parameters:
                with Parallel(n_jobs=self.setup.n_jobs, prefer="threads") as parallel:
                    tables_file = parallel(
                        delayed(add_smoothed_value)(i, j)
                        for i, j in zip(tables_file, repeat(par))
                    )
            ...
            try:
                ...
                with Parallel(n_jobs=self.setup.n_jobs, prefer="threads") as parallel:
                    tables_file = parallel(
                        delayed(interpolate_classification)(i, j)
                        for i, j in zip(tables_file, tables_class_file)
                    )
            except FileNotFoundError:
                pass
            ...
            [
                t.add_column(
                    (t["MAG_APER"] + t["MAG_APER_COR_INTERP"]), name="MAG_APER_MATCHED"
                )
                for t in tables_file
            ]
            ...
            # Compute zero points
            for table_hdu, idx_table_hdu in zip(
                tables_file, self.iter_data_hdu[idx_file]
            ):
                add_zp_2mass(
                    table_hdu,
                    table_2mass=table_master,
                    key_ra=self._key_ra,
                    key_dec=self._key_dec,
                    mag_lim_ref=master_phot.mag_lim(passband=self.passband[idx_file]),
                    method="weighted",
                    passband_2mass=master_phot.translate_passband(
                        self.passband[idx_file][0]
                    ),
                    columns_mag=columns_mag,
                    columns_magerr=columns_magerr,
                )
                ...
                # Add correction factor for the main calibrated mag measurement
                sc1 = SkyCoord(
                    table_hdu[self._key_ra], table_hdu[self._key_dec], unit="degree"
                )
                sc2 = SkyCoord(
                    table_master["RAJ2000"], table_master["DEJ2000"], unit="degree"
                )
                ...
                zp_auto = get_zeropoint(
                    skycoord1=sc1,
                    mag1=table_hdu["MAG_AUTO_CAL"],
                    skycoord2=sc2,
                    method="all",
                    mag2=table_master[pb],
                    mag_limits_ref=master_phot.mag_lim(
                        passband=self.passband[idx_file]
                    ),
                )
                zp_aper = get_zeropoint(
                    skycoord1=sc1,
                    mag1=table_hdu["MAG_APER_MATCHED_CAL"],
                    skycoord2=sc2,
                    method="all",
                    mag2=table_master[pb],
                    mag_limits_ref=master_phot.mag_lim(
                ...
                for par in parameters:
                    add_smoothed_value(
                        table=table_hdu, parameter=par, n_neighbors=150, max_dis=1800
                    )
                ...
                for attr in ["zp", "zperr"]:
                    for key, val in getattr(table_hdu, attr).items():
                        if attr == "zp":
                            ...
                            comment = "Standard error of ZP (mag)"
                        add_float_to_header(
                            header=table_hdulist[idx_table_hdu].header,
                            key=key,
                            value=val,
                            comment=comment,
                            decimals=4,
            ...
            # QC plot
            if self.setup.qc_plots:
                pcsc = PhotometricCalibratedSextractorCatalogs(
                    setup=self.setup, file_paths=outpaths[-1]
                )
