...
class SkyImagesProcessedScience(SkyImagesProcessed):
    ...
    def resample(self):
        ...
        for idx_file in range(self.n_files):
            ...
            # Run Swarp
            run_command_shell(cmd=cmds[idx_file], silent=True, raise_on_error=True)
...
class SkyImagesResampled(SkyImagesProcessed):
    ...
    def build_tile(self):
        ...
        if not (
            ...
            # Verify coadd and weight against the size the tile header implies
            # (float32 data is a strict lower bound on the file size): SWarp
            # can exit 0 yet leave a truncated file that astropy only rejects
            # much later with a cryptic mmap error.
            try:
                hdr = fits.Header.fromtextfile(self.setup.path_coadd_header)
                size_min = hdr["NAXIS1"] * hdr["NAXIS2"] * 4
            except (OSError, KeyError):
                size_min = 10 * 1024 * 1024
            for path in (path_coadd_tmp, path_weight_tmp):
                size = os.path.getsize(path) if os.path.isfile(path) else 0
                if size < size_min:
                    raise PipelineError(
                        f"SWarp tile output missing or truncated: {path} "
                        f"({size:,} of >= {size_min:,} expected bytes). "
                        f"stderr: {stderr}",
                        logger=log,
                    )
