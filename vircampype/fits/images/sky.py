        ...
        for idx_file in range(self.n_files):
            ...
            for idx_hdr in range(len(self.headers_data[idx_file])):
                ...
                for kw, func in zip(keywords, mod_func):
                    ...
                    hdr.set(f"HIERARCH PYPE BACKUP {kw}", value=hdr[kw], comment="Value before IC")
...
class SkyImagesResampled(SkyImagesProcessed):
    ...
    def build_tile(self):
        ...
        if not check_file_exists(file_path=self.setup.path_coadd, silent=self.setup.silent) \
            ...
            astirms = np.sqrt(np.mean(astrms_dict["ASTIRMS1"])**2 + np.mean(astrms_dict["ASTIRMS1"])**2) * 3_600_000
            astrrms = np.sqrt(np.mean(astrms_dict["ASTRRMS1"])**2 + np.mean(astrms_dict["ASTRRMS1"])**2) * 3_600_000
