    ...
    def build_class_star_library(self):
        ...
        for idx_file in range(self.n_files):
            ...
            for fidx in range(len(fwhm_range)):
                add_float_to_header(
                    header=header_prime,
                    key="HIERARCH PYPE CS FWHM {0}".format(fidx + 1),
        ...
        for idx_file in range(self.n_files):
            ...
            if self.setup.fix_vircam_headers:
                fix_vircam_headers(
                    ...
                    reset_wcs=self.setup.reset_wcs,
                )
...
class SkyImagesResampled(SkyImagesProcessed):
    ...
    def build_stacks(self):
        ...
        for idx_split in range(len(split)):
            ...
            path_stack = "{0}{1}_{2:02d}.stack.fits".format(
                self.setup.folders["stacks"], self.setup.name, oidx
            )
            ...
            for idx_data_hdu, idx_iter_hdu in zip(
                ...
                cmd = "{0} {1} -c {2} {3}".format(
                    sws.bin,
                    " ".format(idx_data_hdu).join(paths_full_mod),
                    sws.default_config,
                    ss,
                    idx_data_hdu,
                )
            ...
            for idx in range(len(arcnames)):
                prhdr_stk.set(
                    "HIERARCH PYPE ARCNAME {0:02d}".format(idx), arcnames[idx]
                )
    ...
    def coadd_statistics_stacks(self, mode):
        ...
        # Processing info
        print_header(
            header="STACKS STATISTICS {0}".format(mode.upper()),
            silent=self.setup.silent,
            left=None,
            right=None,
        )
