...
class VircamScienceImages(ScienceImages):
    ...
    def resample_pawprints(self, header_coadd=None):
        ...
        ss = yml2config(path=self._swarp_preset_pawprints_path,
                        ...
                        nthreads=self.setup["misc"]["n_threads"], resample_suffix=self._swarp_resample_suffix,
        ...
        cmds = ["{0} -c {1} {2} -WEIGHT_IMAGE {3} -RESAMPLE_DIR {4} {5}"
                "".format(self.bin_swarp, self._swarp_default_config, path_image, weight, path, ss)
                for path_image, weight, path in
                zip(self.full_paths, self.get_master_weight().full_paths, self.file_directories)]
