...
def mmm(
    ...
    METHOD:
     ...
     REVISION HISTORY:
           ...
           Fixed a few Python style conventions                         S. Meingast                April,    2021
    """

    if nan:
        sky_vector = sky_vector[np.isfinite(sky_vector)]
    ...
    while redo:
        ...
        redo = False
        newmin = minimm
        if sky[newmin + 1] >= cut1:
            tst_min = 1  # Is minimm+1 above current CUT?
        else:
            tst_min = 0
        if (newmin == -1) and tst_min:
            done = 1  # Are we at first pixel of SKY?
        else:
            done = 0
        if not done:
            if newmin > 0:
                ...
                skyind = 0
            if (sky[skyind] < cut1) and tst_min:
                done = 1
        if not done:
            istep = 1 - 2 * int(tst_min)
            while not done:
                newmin = newmin + istep
                if (newmin == -1) | (newmin == nlast):
                    done = 1
                if not done:
                    if (sky[newmin] <= cut1) and (sky[newmin + 1] >= cut1):
                        done = 1
        ...
        newmax = maximm
        if sky[maximm] <= cut2:
            tst_max = 1  # Is current maximum below upper cut?
        else:
            tst_max = 0
        if (maximm == nlast) and tst_max:
            done = 1
        else:
            done = 0  # Are we at last pixel of SKY array?
        if not done:
            if maximm + 1 < nlast:
                skyind = maximm + 1
            else:
                skyind = nlast
            if tst_max and (sky[skyind] > cut2):
                done = 1
        if not done:  # keep incrementing newmax
            istep = -1 + 2 * int(tst_max)  # Increment up or down?
            while not done:
                newmax = newmax + istep
                if (newmax == nlast) or (newmax == -1):
                    done = 1
                if not done:
                    if (sky[newmax] <= cut2) and (sky[newmax + 1] >= cut2):
                        done = 1
